...
<tr><td>p_to_depth.py</td><td>Converts water and air pressure timeseries data to a calculated water depth.</td><td>Overlapping timeseries data for absolute air pressure (from combine_weather_stations.py) plus water pressure (from HOBO log data) csv files.</td><td>A density value must also be assumed.</td></tr>
<tr><td>timeseries_plots.py</td><td>Creates timeseries plots for compiled GSL data.</td><td>Data files for lake elevation, HOBO logs, field metadata</td><td></td></tr>
